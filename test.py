...
parser.add_argument('--resume', type=str, default='./model_data_2/L175__C1_5__r0_125/best_model.pth', metavar='PATH', help='path to latest checkpoint(default: None)')
...
def main():
    ...
    for step, (x, y) in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            # loss, MSE = My_loss(output_en, output_de, b_y, a, 10, wd)
        ...
        print('step:', step, "test-psnr:", psnr, "ssim:", ssim_io)
        l1 += ll1
        l2 += ll2
        l3 += ll3
        l4 += ll4
    ...
    print("rate base_ = ", args.rate_base, file=f)
    print("error rate = ", args.all_error, file=f)
    print("loss-----a = ", a, file=f)
    print("wd = ", wd, file=f)
    print('PSNR =', psnr_avg.tolist(), file=f)
    print('SSIM =', ssim_avg.tolist(), file=f)
    print('homologous =', "[", l1.tolist(), ",", l2.tolist(), ",", l3.tolist(), ",", l4.tolist(), ",", l5.tolist(), ",", l6.tolist(), ",", l7.tolist(), "]", file=f)  # 输出连续情况
    print("AGCT_rate =", AGCT_rate.tolist(), file=f)
    print("best epoch:", start_epoch, file=f)
    print('finish')
    f.close()
