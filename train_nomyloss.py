...
parser.add_argument('--rate_base', type=float, default=0.5, help='base per pixel')
...
parser.add_argument('--save_dir', type=str, default='./C1__L1__r0_5__nomyloss', metavar='N', help='model saving directory')
parser.add_argument('--resume', type=str, default='./C1__L1__r0_5__nomyloss/checkpoint.pth', metavar='PATH', help='path to latest checkpoint(default: None)')
parser.add_argument('--result_dir', type=str, default='./Result__C1__L1__r0_5__nomyloss', metavar='DIR', help='output directory')
parser.add_argument('--a_loss', type=int, default=200, help='loss function parameter')
parser.add_argument('--b_loss', type=int, default=1, help='loss function parameter')
...
def test(model, optimizer, test_dataloader, epoch, best_test_loss):
    ...
    with torch.no_grad():
        for step, (x, y) in enumerate(test_dataloader):
            ...
            test_loss = loss_fn(output_de, b_y)
            test_avg_loss += test_loss

            if step % (len(test_dataloader) // 2) == 0:
                print("epoch={},{}/{}of valid, mse loss={}".format(epoch, step, len(test_dataloader), test_loss.item()))
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        #    if p['lr'] > 0.00001:
        #        p['lr'] = 0.01 / (2 ** (math.floor(epoch / 5)))
        for step, (x, y) in enumerate(train_dataloader):
            b_x = x.permute((0, 1, 2, 3)) * 255.0
            b_y = x.permute((0, 1, 2, 3)) * 255.0

            encoder_output, channel_en, decoder_output = model(b_x, args.sub_error, args.del_error, args.ins_error, args.rate_base)
            loss= loss_fn(decoder_output, b_y)
            ...
            if step % (len(train_dataloader) // 2) == 0:
                ...
                print('Epoch [{}/{}] : Total-loss = {:.4f} lr= {:.4f}, r={:.4f}, er={:.4f}'
                      .format(epoch + 1, args.epochs, loss, optimizer.state_dict()['param_groups'][0]['lr'], args.rate_base, args.all_error),
